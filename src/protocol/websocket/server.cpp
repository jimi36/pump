	...
	namespace protocol {
		namespace websocket {
			...
			bool server::start(
				...
				acceptor_ = transport::tls_acceptor::create_instance_with_file(crtfile, keyfile, listen_address);
				if (acceptor_->start(sv, cbs) != transport::ERROR_OK)
				{
					acceptor_.reset();
					return false;
			...
			bool server::__handle_upgrade_request(http::connection_ptr conn, http::c_request_ptr req)
			{
				...
				{
					send_http_error_response(conn, 400, "Upgrade header is not found or invalid");
					return false;
				}
				...
				{
					send_http_error_response(conn, 400, "Connection header is not found or invalid");
					return false;
				}
				...
				{
					send_http_error_response(conn, 400, "Sec-WebSocket-Version header is not found or invalid");
					return false;
				}
				...
				{
					send_http_error_response(conn, 400, "Sec-WebSocket-Key header is not found or invalid");
					return false;
				}

				std::vector<std::string> protocs;
				header->get("Sec-WebSocket-Protocol", protocs);
				std::string protocol = match_protocol(protocs, __get_local_header("Sec-WebSocket-Protocol"));
			...
			const std::string& server::__get_local_header(
