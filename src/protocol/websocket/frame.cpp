	...
	namespace protocol {
		namespace websocket {
			...
			int32 decode_frame_header(c_block_ptr b, uint32 size, frame_header_ptr hdr)
			{
				...
				if (!r.read(4, &tmp32)) return 0;
				hdr->optcode = tmp32;

				if (!r.read(1, &tmp32)) return 0;
				hdr->mask = tmp32;

				if (!r.read(7, &tmp32)) return 0;
				hdr->payload_len = tmp32;
				if (hdr->payload_len == 126)
				{
					if (!r.read(16, &tmp32)) return 0;
					hdr->ex_payload_len = tmp32;
				}
				else if (hdr->payload_len == 127)
				{
					if (!r.read(64, &tmp64)) return 0;
					hdr->ex_payload_len = tmp64;
				}

				if (hdr->mask == 1)
				{
					if (!r.read(8, &hdr->mask_key[0])) return 0;
					if (!r.read(8, &hdr->mask_key[1])) return 0;
					if (!r.read(8, &hdr->mask_key[2])) return 0;
					if (!r.read(8, &hdr->mask_key[3])) return 0;
				}
			...
			int32 encode_frame_header(c_frame_header_ptr hdr, block_ptr b, uint32 size)
			{
				...
				if (!w.write(1, hdr->fin)) return 0;

				if (!w.write(1, hdr->rsv1)) return 0;
				if (!w.write(1, hdr->rsv2)) return 0;
				if (!w.write(1, hdr->rsv3)) return 0;

				if (!w.write(4, hdr->optcode)) return 0;

				if (!w.write(1, hdr->mask)) return 0;

				if (!w.write(7, hdr->payload_len)) return 0;
				...
				if (hdr->mask == 1)
				{
					if (!w.write(8, hdr->mask_key[0])) return 0;
					if (!w.write(8, hdr->mask_key[1])) return 0;
					if (!w.write(8, hdr->mask_key[2])) return 0;
					if (!w.write(8, hdr->mask_key[3])) return 0;
				}
