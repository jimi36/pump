...
const static int64_t MAX_RTT = int64_t(1) << 60;
...
bool rtt_state::set_initial_rtt(int64_t r) {
    if (inited_) {
        return false;
    }
...
void rtt_state::update(int64_t sample, int64_t ack_delay, int64_t now) {
    ...
    if (!inited_) {
        ...
        mean_deviation_ = (float32_t)mean_deviation_ * (1 - RTT_BETA) +
                          (float32_t)abs(smoothed_ - sample) * RTT_BETA;
        smoothed_ =
            (float32_t)smoothed_ * (1 - RTT_ALPHA) + (float32_t)sample * RTT_ALPHA;
    }
}

int64_t rtt_state::get_pto(bool include_max_ack_delay) {
    if (smoothed_ == 0) {
        return DEF_RTT * 2;
    }
