...
static uint32_t __ack_range_count(const ack_frame *frame) {
    ...
    max_ack_frame_len -=
        varint_length(frame->ack_ranges[0].largest);  // reduce length of largest_ack.
    max_ack_frame_len -= varint_length(
        __encode_ack_delay(frame->ack_delay,
                           frame->ack_delay_export));  // reduce length of ack_delay.
    ...
    for (uint32_t i = 1; i < (uint32_t)frame->ack_ranges.size(); i++) {
        uint64_t gap =
            frame->ack_ranges[i - 1].smallest - frame->ack_ranges[i].largest - 2;
...
bool pack_ack_frame(const ack_frame *frame, io_buffer *iob) {
    ...
    if (!varint_encode(frame->ack_ranges[0].largest, iob) ||
        !varint_encode(ack_delay, iob) || !varint_encode(range_count, iob) ||
    ...
    for (uint32_t i = 1; i <= range_count; i++) {
        uint64_t gap =
            frame->ack_ranges[i - 1].smallest - frame->ack_ranges[i].largest - 2;
        uint64_t len = frame->ack_ranges[i].largest - frame->ack_ranges[i].smallest;
        if (!varint_encode(gap, iob) || !varint_encode(len, iob)) {
            return false;
        }
    }

    if (tp == FT_ACK_ECN) {
        if (!varint_encode(frame->ect0, iob) || !varint_encode(frame->ect1, iob) ||
...
bool unpack_ack_frame(io_buffer *iob, ack_frame *frame) {
    ...
    for (uint64_t i = 0; i < range_count; i++) {
        uint64_t gap, len;
        if (!varint_decode(iob, &gap) || !varint_decode(iob, &len)) {
            return false;
        }
    ...
    if (tp == FT_ACK_ECN) {
        if (!varint_decode(iob, &frame->ect0) || !varint_decode(iob, &frame->ect1) ||
...
bool pack_reset_stream_frame(const reset_stream_frame *frame, io_buffer *iob) {
    ...
    if (!varint_encode(frame->stream_id, iob) || !varint_encode(frame->error_code, iob) ||
...
bool pack_crypto_frame(const crypto_frame *frame, io_buffer *iob) {
    ...
    if (!varint_encode(frame->offset, iob) || !varint_encode(frame->data.size(), iob)) {
        return false;
    }
...
bool pack_max_stream_data_frame(const max_stream_data_frame *frame, io_buffer *iob) {
    ...
    if (!varint_encode(frame->stream_id, iob) || !varint_encode(frame->max, iob)) {
        return false;
    }
...
bool unpack_max_stream_data_frame(io_buffer *iob, max_stream_data_frame *frame) {
    ...
    if (!varint_decode(iob, &frame->stream_id) || !varint_decode(iob, &frame->max)) {
        return false;
    }
...
bool pack_stream_data_blocked_frame(const stream_data_blocked_frame *frame,
                                    io_buffer *iob) {
...
bool pack_new_connection_id_frame(const new_connection_id_frame *frame, io_buffer *iob) {
    ...
    if (!varint_encode(frame->seq, iob) || !varint_encode(frame->retire_prior_to, iob)) {
        return false;
    }
...
bool pack_retire_connection_id_frame(const retire_connection_id_frame *frame,
                                     io_buffer *iob) {
...
bool unpack_retire_connection_id_frame(io_buffer *iob,
...
int32_t length_connection_close_frame(const connection_close_frame *frame) {
    return 1 + varint_length(frame->ec) + varint_length(frame->reason.size()) +
           frame->reason.size();
}
