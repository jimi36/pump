...
static bool __pack_preferred_address(const transport_preferred_address *preferred_address,
                                     io_buffer *iob) {
...
static bool __unpack_preferred_address(io_buffer *iob,
    ...
    block_t len;
    if (!iob->read(&len) || !preferred_address->id.read_from(iob, len)) {
        return false;
    }
...
bool pack_parameters(stream_initiator_type initiator,
    ...
    if (initiator == server_initiator && params->stateless_reset_token.size() == 16) {
        if (!varint_encode(PARAM_STATELESS_RESET_TOKEN, iob) || !varint_encode(16, iob) ||
    ...
    if (initiator == server_initiator && params->preferred_address != nullptr) {
        if (!varint_encode(PARAM_PREFERRED_ADDRESS, iob) ||
            !varint_encode(4 + 2 + 16 + 2 + 1 + params->preferred_address->id.length() +
                               16,
                           iob) ||
    ...
    if (!varint_encode(PARAM_INITIAL_SOURCE_CONNECTION_ID, iob) ||
        !varint_encode(varint_length(params->initial_source_connection_id.length()),
                       iob) ||
    ...
    if (initiator == server_initiator &&
        params->retry_source_connection_id.length() > 0) {
        if (!varint_encode(PARAM_RETRY_SOURCE_CONNECTION_ID, iob) ||
            !varint_encode(varint_length(params->retry_source_connection_id.length()),
                           iob) ||
