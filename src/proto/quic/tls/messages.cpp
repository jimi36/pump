...
namespace tls {
    ...
    bool pack_client_hello_message(void *msg, io_buffer *iob) {
        ...
        if (!raw->server_name.empty()) {
            if (!write_i16_to_iob(TLS_EXTENSION_SERVER_NAME, iob) ||
                ...
                !write_i16_to_iob(raw->server_name.size(), iob) || // server name length
        ...
        if (!raw->alpns.empty()) {
            ...
            for (auto &alpn : raw->alpns) {
                if (!write_i8_to_iob(alpn.size(), iob) ||
                    !write_string_to_iob(alpn, iob)) {
                    return false;
                }
            }
    ...
    bool unpack_client_hello_message(io_buffer *iob, void *msg) {
        ...
        auto ex_iob = io_buffer::create_by_refence(nullptr, 0);
    ...
    bool pack_server_hello_message(void *msg, io_buffer *iob) {
        ...
        if (!raw->session_id.empty() && 
            !write_string_to_iob(raw->session_id, iob)) {
            return false;
        }
        ...
        if (!raw->scts.empty()) {
            ...
            for (auto &sct : raw->scts) {
                if (!write_i16_to_iob(sct.size(), iob) ||
                    !write_string_to_iob(sct, iob)) {
                    return false;
                }
            }
            uint16_t len = iob->size() - len_pos - 2;
        ...
        if (raw->supported_version != 0) {
            if (!write_i16_to_iob(TLS_EXTENSION_SUPPORTED_VERSIONS, iob) ||
                ...
                !write_i16_to_iob(raw->supported_version, iob)) {
                return false;
            }
        }
        ...
        if (raw->selected_group != 0) {
            if (!write_i16_to_iob(TLS_EXTENSION_KEY_SHARE, iob) ||
                !write_i16_to_iob(2, iob) ||
                !write_i16_to_iob(raw->selected_group, iob)) {
                return false;
            }
        }
    ...
    bool pack_new_session_ticket_tls13_message(void *msg, io_buffer *iob) {
        ...
        if (!raw->nonce.empty() &&
            !write_string_to_iob(raw->nonce, iob)) {
            return false;
        }
        ...
        if (!raw->label.empty() &&
            !write_string_to_iob(raw->label, iob)) {
            return false;
        }
    ...
    bool pack_encrypted_extensions_message(void *msg, io_buffer *iob) {
        ...
        for (auto &ext : raw->additional_extensions) {
            if (!write_i16_to_iob(ext.type, iob) ||
                !write_i16_to_iob(ext.data.size(), iob) ||
    ...
    bool pack_certificate_message(void *msg, io_buffer *iob) {
        ...
        for (auto &cert : raw->certificates) {
            // Pack certificate.
            if (!write_i24_to_iob(cert.size(), iob) ||
                !write_string_to_iob(cert, iob)) {
                return false;
            }
        }
    ...
    bool pack_certificate_req_message(void *msg, io_buffer *iob) {
        ...
        for (auto &auth : raw->certificate_authorities) {
            if (!write_i16_to_iob(auth.size(), iob) ||
                !write_string_to_iob(auth, iob)) {
                return false;
            }
        }
        uint16_t certificate_authorities_len = iob->size() - certificate_authorities_len_pos - 2;
        __pack_i16((block_t*)iob->data() + certificate_authorities_len_pos, certificate_authorities_len);
    ...
    bool pack_certificate_req_tls13_message(void *msg, io_buffer *iob) {
        ...
        if (!raw->supported_signature_algorithms_certs.empty()) {
            if (!write_i16_to_iob(TLS_EXTENSION_SIGNATURE_ALGORITHMS_CERT, iob) ||
                !write_i16_to_iob(2 + raw->supported_signature_algorithms_certs.size() * 2, iob) || 
                !write_i16_to_iob(raw->supported_signature_algorithms_certs.size() * 2, iob)) {
                return false;
            }
