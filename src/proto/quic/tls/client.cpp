...
alert_code client_handshaker::handshake(handshake_message *msg) {
    ...
    case TLS_MSG_CERTIFICATE_REQUEST:
        PUMP_DEBUG_LOG("quic client handshaker handle certificate request "
                       "tls13 message");
...
bool client_handshaker::__send_client_hello() {
    ...
    auto hello = (client_hello_message *)hello_->msg;
...
alert_code client_handshaker::__handle_server_hello(handshake_message *msg) {
    if (status_ != HANDSHAKE_CLIENT_HELLO_SEND && status_ != HANDSHAKE_RETRY_HELLO_SEND) {
    ...
    auto client_hello = (client_hello_message *)hello_->msg;
    ...
    if (server_hello->is_support_ocsp_stapling ||
        ...
        !server_hello->renegotiation_info.empty() || !server_hello->alpn.empty() ||
...

    {
        ...
        // PUMP_DEBUG_LOG("client handshaker handshake_secret_base64: %s",
    ...
    // codec::base64_encode(session_.client_secret); PUMP_DEBUG_LOG("client
    ...
    // codec::base64_encode(session_.server_secret); PUMP_DEBUG_LOG("client
...

    {
        ...
        // PUMP_DEBUG_LOG("client handshaker master_secret_base64: %s",
...
alert_code client_handshaker::__handle_encrypted_extensions(handshake_message *msg) {
    ...
    if (client_hello->is_support_early_data &&
        encrypted_extensions->is_support_early_data) {
...
alert_code client_handshaker::__handle_certificate_verify(handshake_message *msg) {
    ...
    if (!ssl::verify_x509_signature(session_.peer_certs[0],
                                    sign_algo,
                                    hash_algo,
                                    signed_msg,
...
alert_code client_handshaker::__handle_finished(handshake_message *msg) {
    ...
    // PUMP_DEBUG_LOG("client handshaker server verify_data_base64: %s",
    ...
    // codec::base64_encode(session_.traffic_secret); PUMP_DEBUG_LOG("client
    ...
    // PUMP_DEBUG_LOG("client handshaker export_master_secret_base64: %s",
...
alert_code client_handshaker::__send_certificate_verify() {
    ...
    if (!session_.certs.empty()) {
        ...
        auto sign = generate_signed_message(hash_algo,
                                            SERVER_SIGNATURE_CONTEXT,
                                            ssl::sum_hash(transcript_));
...
alert_code client_handshaker::__send_finished() {
    ...
    // PUMP_DEBUG_LOG("client handshaker client verify_data_base64: %s",
    // verify_data_base64.c_str());
...
void client_handshaker::__send_handshake_message(handshake_message *msg,
                                                 bool transcript) {
    if (!pack_handshake_message(msg)) {
        PUMP_DEBUG_LOG("pack handhake message failed: message type %d", msg->tp);
    } else {
        if (transcript) {
            __write_transcript(msg->packed);
        }
