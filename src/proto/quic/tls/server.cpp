...
alert_code server_handshaker::handshake(handshake_message *msg) {
    ...
    case TLS_MSG_CERTIFICATE:
        PUMP_DEBUG_LOG("quic server handshaker handle certificate tls13 message");
        code = __handle_certificate_tls13(msg);
        break;
    case TLS_MSG_CERTIFICATE_VERIFY:
        PUMP_DEBUG_LOG("quic server handshaker handle certificate verify message");
        code = __handle_certificate_verify(msg);
        break;
    case TLS_MSG_FINISHED:
        PUMP_DEBUG_LOG("quic server handshaker handle finished message");
        code = __handle_finished(msg);
        break;
    default:
        PUMP_DEBUG_LOG("quic server handshaker handle unknown message");
...
alert_code server_handshaker::__handle_client_hello(handshake_message *msg) {
    if (status_ != HANDSHAKE_INIT && status_ != HANDSHAKE_HELLO_REQUEST_SEND) {
...
alert_code server_handshaker::__send_server_hello() {
...

    {
        ...
        if (!ssl::gen_ecdhe_shared_key(session_.ecdhe_ctx,
                                       client_hello_->key_shares[0].data,
                                       shared_key)) {
            return ALERT_INTERNAL_ERROR;
        }
        session_.handshake_secret =
            cipher_suite_extract(session_.cipher_suite_ctx, secret, shared_key);
        // auto handshake_secret_base64 =
        // codec::base64_encode(session_.handshake_secret);
        // PUMP_DEBUG_LOG("server handshaker handshake_secret_base64: %s",
        // handshake_secret_base64.c_str());
    }

    session_.client_secret = cipher_suite_device_secret(session_.cipher_suite_ctx,
                                                        session_.handshake_secret,
                                                        CLIENT_HANDSHAKE_TRAFFIC_LABEL,
                                                        transcript_);
    // auto client_secret_base64 =
    // codec::base64_encode(session_.client_secret); PUMP_DEBUG_LOG("server
    // handshaker client_secret_base64: %s", client_secret_base64.c_str());

    session_.server_secret = cipher_suite_device_secret(session_.cipher_suite_ctx,
                                                        session_.handshake_secret,
                                                        SERVER_HANDSHAKE_TRAFFIC_LABEL,
                                                        transcript_);
    // auto server_secret_base64 =
    // codec::base64_encode(session_.server_secret); PUMP_DEBUG_LOG("server
    // handshaker server_secret_base64: %s", server_secret_base64.c_str());

    {
        auto secret = cipher_suite_device_secret(session_.cipher_suite_ctx,
                                                 session_.handshake_secret,
                                                 "derived",
                                                 nullptr);
        session_.master_secret =
            cipher_suite_extract(session_.cipher_suite_ctx, secret, "");
        // auto master_secret_base64 =
        // codec::base64_encode(session_.master_secret);
        // PUMP_DEBUG_LOG("server handshaker master_secret_base64: %s",
        // master_secret_base64.c_str());
    }

    PUMP_DEBUG_LOG("quic server handshaker send server hello message");
...
alert_code server_handshaker::__send_encrypted_extensions() {
    ...
    PUMP_DEBUG_LOG("quic server handshaker send encrypted extensions message");
    __send_handshake_message(msg);
...
alert_code server_handshaker::__send_certificate_request() {
    ...
    PUMP_DEBUG_LOG("quic server handshaker send certificate request tls13 message");
    __send_handshake_message(msg);
...
alert_code server_handshaker::__send_certificate_tls13() {
    ...
    PUMP_DEBUG_LOG("quic server handshaker send certificate tls13 message");
    __send_handshake_message(msg);
...
alert_code server_handshaker::__send_certificate_verify() {
    ...
    auto signed_msg = generate_signed_message(hash_algo,
                                              SERVER_SIGNATURE_CONTEXT,
                                              ssl::sum_hash(transcript_));

    if (!ssl::do_x509_signature(session_.certs[0],
                                sign_algo,
                                hash_algo,
                                signed_msg,
    ...
    // PUMP_DEBUG_LOG("client handshaker verify signature sign: %d hash: %ld
    // msg: %s signature: %s",
    //    sign_algo,
    //    hash_algo,
    ...
    PUMP_DEBUG_LOG("quic server handshaker send certificate verify message");
    __send_handshake_message(msg);
...
alert_code server_handshaker::__send_finished() {
    ...
    auto finished_key =
        hkdf_expand_label(session_.cipher_suite_ctx->algo,
                          session_.server_secret,
                          "",
                          "finished",
                          ssl::hash_digest_length(session_.cipher_suite_ctx->algo));
    finished->verify_data = ssl::sum_hmac(session_.cipher_suite_ctx->algo,
                                          finished_key,
                                          ssl::sum_hash(transcript_));
    // auto verify_data_base64 = codec::base64_encode(finished->verify_data);
    // PUMP_DEBUG_LOG("server handshaker server verify_data_base64: %s",
    // verify_data_base64.c_str());
    ...
    session_.traffic_secret = cipher_suite_device_secret(session_.cipher_suite_ctx,
                                                         session_.master_secret,
                                                         CLIENT_APPLICATION_TRAFFIC_LABEL,
                                                         transcript_);
    // auto traffic_secret_base64 =
    // codec::base64_encode(session_.traffic_secret); PUMP_DEBUG_LOG("server
    // handshaker traffic_secret_base64: %s", traffic_secret_base64.c_str());
    ...
    session_.export_master_secret = cipher_suite_device_secret(session_.cipher_suite_ctx,
                                                               session_.master_secret,
                                                               EXPORTER_LABEL,
                                                               transcript_);
    // auto export_master_secret_base64 =
    // codec::base64_encode(session_.export_master_secret);
    // PUMP_DEBUG_LOG("server handshaker export_master_secret_base64: %s",
    // export_master_secret_base64.c_str());

    PUMP_DEBUG_LOG("quic server handshaker send finished message");
...
alert_code server_handshaker::__handle_certificate_verify(handshake_message *msg) {
    ...
    auto signed_msg = generate_signed_message(hash_algo,
                                              SERVER_SIGNATURE_CONTEXT,
                                              ssl::sum_hash(transcript_));
    if (!ssl::verify_x509_signature(session_.certs[0],
                                    sign_algo,
                                    hash_algo,
                                    signed_msg,
                                    cert_verify->signature)) {
...
alert_code server_handshaker::__handle_finished(handshake_message *msg) {
    ...
    auto finished_key =
        hkdf_expand_label(session_.cipher_suite_ctx->algo,
                          ...
                          "finished",
                          ssl::hash_digest_length(session_.cipher_suite_ctx->algo));
    auto verify_data = ssl::sum_hmac(session_.cipher_suite_ctx->algo,
                                     finished_key,
                                     ssl::sum_hash(transcript_));
    // auto verify_data_base64 = codec::base64_encode(verify_data);
    // PUMP_DEBUG_LOG("server handshaker client verify_data_base64: %s",
    // verify_data_base64.c_str());
...
void server_handshaker::__send_handshake_message(handshake_message *msg,
                                                 bool transcript) {
    if (!pack_handshake_message(msg)) {
