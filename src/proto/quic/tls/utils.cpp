...
std::string cipher_suite_extract(cipher_suite_context *ctx,
                                 const std::string &salt,
                                 const std::string &key) {
    return hkdf_extract(ctx->algo, salt, key);
}

std::string cipher_suite_expand_label(cipher_suite_context *ctx,
                                      const std::string &key,
...
std::string cipher_suite_device_secret(cipher_suite_context *ctx,
                                       const std::string &key,
                                       const std::string &label,
...
std::string generate_signed_message(ssl::hash_algorithm algo,
                                    const std::string &context,
                                    const std::string &msg) {
    std::string signed_msg;
    if (algo == ssl::HASH_UNKNOWN) {
        signed_msg.append((const char *)signature_padding,
                          (size_t)sizeof(signature_padding));
        ...
        if (!ssl::update_hash(hash_ctx,
                              signature_padding,
                              (int32_t)sizeof(signature_padding))) {
