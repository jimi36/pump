...
struct x509_certificate {
    x509_certificate() : key(nullptr), cert(nullptr) {}
#if defined(PUMP_HAVE_OPENSSL)
    EVP_PKEY *key;
    X509 *cert;
#else
...
x509_certificate *generate_x509_certificate(signature_scheme scheme) {
    ...
    case TLS_SIGN_SCHE_ECDSAWITHP521AndSHA512: {
        ...
        if ((xcert->cert = X509_new()) == nullptr) {
            ...
            break;
        } else if (X509_gmtime_adj(X509_get_notAfter(xcert->cert), 365L * 86400) ==
                   nullptr) {
            break;
...
x509_certificate *load_x509_certificate_by_pem(const std::string &cert,
                                               const std::string &key) {
    return load_x509_certificate_by_pem(cert.data(), cert.size(), key.data(), key.size());
}

x509_certificate *load_x509_certificate_by_pem(const block_t *cert,
                                               int32_t cert_size,
                                               const block_t *key,
                                               int32_t key_size) {
    x509_certificate *xcert = nullptr;
...
x509_certificate *load_x509_certificate_by_bin(const std::string &cert,
                                               const std::string &key) {
    return load_x509_certificate_by_bin(cert.data(), cert.size(), key.data(), key.size());
}

x509_certificate *load_x509_certificate_by_bin(const block_t *cert,
                                               int32_t cert_size,
                                               const block_t *key,
                                               int32_t key_size) {
    x509_certificate *xcert = nullptr;
...
bool do_x509_signature(x509_certificate *xcert,
...
bool verify_x509_signature(x509_certificate *xcert,
                           signature_algorithm sign_algo,
                           hash_algorithm hash_algo,
                           const std::string &msg,
