...
// C++17 new(size_t, align_val_t) is not backwards-compatible with older versions of
// macOS, so we can't support over-alignment in this case
...
class MOODYCAMEL_MAYBE_ALIGN_TO_CACHELINE ReaderWriterQueue {
  ...
  public:
...
#endif
    {
        ...
        if (largestBlockSize > MAX_BLOCK_SIZE * 2) {
            ...
            // elements. We also need a spare element in each block to avoid the ambiguity
            // between front == tail meaning "empty" and "full".
            // So the effective number of slots that are guaranteed to be usable at any
            // time is the block size - 1 times the number of blocks - 1. Solving for size
    ...
    template <typename U> bool try_dequeue(U &result) AE_NO_TSAN {
        ...
        // Else
        //     If front block was the tail block when we entered the function, return
        //     false Else advance to next block and dequeue the item there
        ...
        // block. Seems unlikely, but was consistently reproducible in practice. In order
        // to avoid overhead in the common case, though, we do a double-checked pattern
        // where we have the fast path if the front block is not empty, then read the tail
        // block, then re-read the front block and check if it's not empty again, then
        // check if the tail block has advanced.
        ...
        non_empty_front_block:
            ...
            // tailBlock, and we're not the tailBlock, and we did an acquire earlier after
            // reading tailBlock which ensures next is up-to-date on this CPU in case we
            // recently were at tailBlock.
    ...
    //       the block the consumer is removing from until it's completely empty, except
    //       in the case where the producer was writing to the same block the consumer was
...
#endif
    {
        ...
        if (nextBlockTail != blockFront ||
            ...
            if (tailBlock_->next.load() != frontBlock) {
                ...
                // adding new entries there is because if we did, then dequeue would stay
                // in that block, eventually reading the new values, instead of advancing
                // to the next full block (whose values were enqueued first and so should
                // be consumed first).

                fence(memory_order_acquire);  // Ensure we get latest writes if we got the
                                              // latest frontBlock
  ...
  private:
    ...
    struct Block {
        // Avoid false-sharing by putting highly contended variables on their own cache
        // lines
        ...
        char cachelineFiller1[MOODYCAMEL_CACHE_LINE_SIZE - sizeof(weak_atomic<size_t>) -
                              sizeof(size_t)];  // next isn't very contended, but we don't
                                                // want it on the same cache line as tail
                                                // (which is)
