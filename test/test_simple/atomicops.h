...
// implementation of low-level memory barriers, plus a few semi-portable utility macros
// (for inlining and alignment). Also has a basic atomic type (limited to
// hardware-supported atomics with no memory ordering guarantees). Uses the AE_* prefix
// for macros (historical reasons), and the "moodycamel" namespace for symbols.
...
// Provides basic support for atomic variables -- no memory ordering guarantees are
// provided. The guarantee of atomicity is only made for types that already have atomic
// load and store guarantees at the hardware level -- on most platforms this generally
// means aligned pointers and integers (only).
namespace moodycamel {
...
#pragma warning(push)
#pragma warning( \
    disable : 4100)  // Get rid of (erroneous) 'unreferenced formal parameter' warning
#endif
    template <typename U> AE_NO_TSAN weak_atomic(U &&x) : value(std::forward<U>(x)) {}
#ifdef __cplusplus_cli
    // Work around bug with universal reference/nullptr combination that only appears when
    // /clr is on
