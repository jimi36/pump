...
#ifndef pump_proto_quic_defines_h
#define pump_proto_quic_defines_h
...
const static int32_t UDP_RECEIVE_BUFFER_SIZE = (1 << 20) * 2;
...
// If consumed receive window scale higher than the threshold, update receive window.
const static float32_t RECV_WINDOW_UPDATE_THRESHOLD = 0.25;

// ConnectionFlowControlMultiplier determines how much larger the connection flow
// control windows needs to be relative to any stream's flow control window. This
// is the value that Chromium is using.
const static float32_t FLOW_CONTROL_MULTIPLIER = 1.5;

// Default initial stream-level flow control window for receiving.
const static int64_t DEF_STREAM_RECV_WINDOW_SIZE = (1 << 10) * 512;
// Default initial stream-level flow control window for receiving.
const static int64_t DEF_MAX_STREAM_RECV_WINDOW_SIZE = (1 << 20) * 6;

// Default initial connection-level flow control window for receiving.
const static int64_t DEF_CONNECTION_RECV_WINDOW_SIZE =
    DEF_STREAM_RECV_WINDOW_SIZE * FLOW_CONTROL_MULTIPLIER;
// Default initial connection-level flow control window for receiving.
const static int64_t DEF_MAX_CONNECTION_RECV_WINDOW_SIZE = (1 << 20) * 15;
...
// Default time to keeping closed sessions around in order to retransmit the
// CONNECTION_CLOSE with 5s. After this time all information about the old
...
// Max stream count value that can be sent in MAX_STREAMS frames and as the stream
// count. in the transport parameters.
...
const int64_t TIMER_GRANULARITY = 1;
