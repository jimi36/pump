...
/*********************************************************************************
 * The hello request message MAY be sent by the server at any time. Hello
 *request is a simple notification that the client should begin the
 *negotiation process a new by sending a client hello message when convenient.
 ...
 *have transmission precedence over application data, it is expected that the
 *negotiation will begin before no more than a few records are received from
 *the client.  If the server sends a hello request but does not receive a
...
/*********************************************************************************
 * When client first connects to a server, it is REQUIRED to send the
 *ClientHello as its first TLS message. The client will also send a
 *ClientHello when the server has responded to its ClientHello with a
 *HelloRetryRequest. https://tools.ietf.org/html/rfc8446#section-4.1.2
 ********************************************************************************/
struct client_hello_message {
    // In TLS 1.3, the client indicates its version preferences in the
    // "supported_versions" extension and the legacy_version field MUST be set
    // to 0x0303, which is the version number for TLS 1.2. TLS 1.3
    // ClientHellos are identified as having a legacy_version of 0x0303 and a
    ...
    // networks. This extension allows for such information to be sent in the
    // TLS handshake, saving roundtrips and resources.
    ...
    // supported curves. https://tools.ietf.org/html/rfc4492#section-5.1.2
    std::vector<point_format_type> supported_point_formats;

    // Extension field.
    // If the client possesses a ticket that it wants to use to resume a
    // session, then it includes the ticket in SessionTicket extension in the
    ...
    // client is not prepared to receive a ticket in the NewSessionTicket
    // handshake message, then it MUST NOT include a SessionTicket extension
    // unless it is sending a non-empty ticket it received through some other
    // means from the server. https://tools.ietf.org/html/rfc5077#section-3.2
    ...
    // server which sign/hash algorithm pairs may be used in digital
    ...
    // binding to the enclosing TLS connection (if any) for which the
    // renegotiation is being performed.
    // https://tools.ietf.org/html/rfc5746#section-3.2
    bool is_support_renegotiation_info;
    std::string renegotiation_info;

    // Extension field.
    // A new extension "application_layer_proto_negotiation" is defined and
    // MAY be included by the client in its "ClientHello" message.
    // https://tools.ietf.org/html/rfc7301#section-3.1
    std::vector<std::string> alpns;

    // Extension field.
    // The SCT can be sent during the TLS handshake using a TLS extension with
    // type "signed_certificate_timestamp". Clients that support the extension
    // SHOULD send a ClientHello extension with the appropriate type and empty
    // "extension_data".
    ...
    // negotiate https://tools.ietf.org/html/rfc8446#section-4.2.1
    std::vector<version_type> supported_versions;

    // Extension field.
    // Cookies serve two primary purposes:
    // - Allowing server to force the client to demonstrate reachability at
    // their apparent network address (thus providing a measure of DoS
    // protection). This is primarily useful for non-connection-oriented
    // transports (see [RFC6347] for an example of this).
    // - Allowing server to offload state to the client, thus allowing it to
    // send a HelloRetryRequest without storing any state. The server can do
    // this by storing the hash of the ClientHello in the HelloRetryRequest
    // cookie (protected with some suitable integrity protection algorithm).
    // https://tools.ietf.org/html/rfc8446#section-4.2.2
    std::string cookie;

    // Extension field.
    // The "key_share" extension contains the endpoint's cryptographic
    // parameters. Clients MAY send an empty client_shares vector in order to
    // request group selection from the server, at the cost of an additional
    // round trip. https://tools.ietf.org/html/rfc8446#section-4.2.8
    ...
    // so, the impact will just be that the client's attempts at resumption
    // fail. https://tools.ietf.org/html/rfc8446#section-4.2.9
    ...
    // PSK key establishment. The "pre_shared_key" extension MUST be the last
    // extension in the ClientHello (this facilitates implementation as
    // described below). Servers MUST check that it is the last extension and
...
struct server_hello_message {
    // In previous versions of TLS, this field was used for version
    // negotiation and represented the selected version number for the
    // connection. Unfortunately, some middleboxes fail when presented with
    // new values. In TLS 1.3, the TLS server indicates its version using the
    ...
    // parties must proceed directly to the Finished messages. Otherwise, this
    // field will contain a different value identifying the new session. The
    // server may return an empty session_id to indicate that the session will
    // not be cached and therefore cannot be resumed. If a session is resumed,
    // it must be resumed using the same cipher suite it was originally
    // negotiated with. Note that there is no requirement that the server
    // resume any session even if it had formerly provided a session_id.
    // Clients MUST be prepared to do a full negotiation -- including
    // negotiating new cipher suites -- during any handshake.
    // https://tools.ietf.org/html/rfc5077
    // https://tools.ietf.org/html/rfc5246#section-7.4.1.3
    // https://tools.ietf.org/html/rfc8446#section-4.1.2
    std::string session_id;
    ...
    // in ClientHello compression_methods. For resumed sessions, this field is
    ...
    // client along with their certificate. If OCSP is requested, they SHOULD
    // use the information contained in the extension when selecting an OCSP
    ...
    // NewSessionTicket handshake message. The server MUST send this extension
    // in the ServerHello if it wishes to issue a new ticket to the client
    // using the NewSessionTicket handshake message. The server MUST NOT send
    // this extension if it does not receive one in the ClientHello. If the
    // server fails to verify the ticket, then it falls back to performing a
    ...
    // Extension field.
    // A new TLS extension, "renegotiation_info", which contains a
    // cryptographic binding to the enclosing TLS connection (if any) for
    // which the renegotiation is being performed.
    // https://tools.ietf.org/html/rfc5746#section-3.2
    bool is_support_renegotiation_info;
    std::string renegotiation_info;
    ...
    // proto name that it does not recognize. A new ServerHello extension
    // type. https://tools.ietf.org/html/rfc7301#section-3.1
    std::string alpn;

    // Extension field.
    // The SCT can be sent during the TLS handshake using a TLS extension with
    // type "signed_certificate_timestamp". Servers MUST only send SCTs to
    // clients who have indicated support for the extension in the
    // ClientHello. https://tools.ietf.org/html/rfc6962#section-3.3.1
    ...
    // (TLS 1.2). https://tools.ietf.org/html/rfc8446#section-4.2.1
    version_type supported_version;

    // Extension field.
    // If using (EC)DHE key establishment, servers offer exactly one
    // KeyShareEntry in the ServerHello. This value MUST be in the same group
    // as the KeyShareEntry value offered by the client that the server has
    // selected for the negotiated key exchange. Servers MUST NOT send a
    // KeyShareEntry for any group not indicated in the client's
    ...
    // verify that (1) the selected_group field corresponds to a group which
    // was provided in the "supported_groups" extension in the original
    // ClientHello and (2) the selected_group field does not correspond to a
    // group which was provided in the "key_share" extension in the original
    // ClientHello. If either of these checks fails, then the client MUST
    // abort the handshake with an "illegal_parameter" alert. Otherwise, when
    // sending the new ClientHello, the client MUST replace the original
    // "key_share" extension with one containing only a new KeyShareEntry for
    ...
    // not validate, the server MUST abort the handshake. Servers SHOULD NOT
    // attempt to validate multiple binders; rather, they SHOULD select a
    // single PSK and validate solely the binder that corresponds to that PSK.
    // See Section 8.2 and Appendix E.6 for the security rationale for this
    // requirement. In order to accept PSK key establishment, the server sends
    // a "pre_shared_key" extension indicating the selected identity.
    // https://tools.ietf.org/html/rfc8446#section-4.2.11
    ...
    // ClientHello. Since the server is not storing any state, this will
    // appear as if it were the first message to be received. Servers
    // operating statelessly MUST ignore these records.
    // https://tools.ietf.org/html/rfc8446#section-4.2.2
    std::string cookie;
};
 ...
 * ChangeCipherSpec message. This message MUST be sent if the server included
 *a SessionTicket extension in the ServerHello. This message MUST NOT be sent
 *if the server did not include a SessionTicket extension in the ServerHello.
 *This message is included in the hash used to create and verify the Finished
 *message. In the case of a full handshake, the server MUST verify the
 *client's Finished message before sending the ticket. The client MUST NOT
 *treat the ticket as valid until it has verified the server's Finished
 *message. If the server determines that it does not want to include a ticket
 *after it has included the SessionTicket extension in the ServerHello , then
 ...
 *MAY send a NewSessionTicket message. This message creates a unique
 *association between the ticket value and a secret PSK derived from the
 *resumption master secret. https://tools.ietf.org/html/rfc8446#section-4.6.1
 ********************************************************************************/
struct new_session_ticket_tls13_message {
    ...
    // use any value greater than 604800 seconds (7 days). The value of zero
    // indicates that the ticket should be discarded immediately. Clients MUST
    // NOT cache tickets for longer than 7 days, regardless of the
    // ticket_lifetime, and MAY delete tickets valid earlier based on local
    // policy.  A server MAY treat a ticket as for a shorter period of time
    // than what is stated in the ticket_lifetime.
    // https://tools.ietf.org/html/rfc8446#section-4.6.1
    uint32_t lifetime;

    // A securely generated, random 32-bit value that is used to obscure the
    // age of the ticket that the client includes in the added to this
    // "pre_shared_key" extension. The client-side ticket age is value modulo
    // 2^32 to obtain the value that is transmitted by the client. The server
    // MUST generate a fresh value for each ticket it sends.
    // https://tools.ietf.org/html/rfc8446#section-4.6.1
    uint32_t age_add;
    ...
    // The value of the ticket to be used as the PSK identity. The ticket
    // itself is an opaque label. It MAY be either a database lookup key or a
    // self-encrypted and self-authenticated value.
    // https://tools.ietf.org/html/rfc8446#section-4.6.1
    std::string label;

    // A set of extension values for the ticket. The "Extension" format is
    // defined in Section 4.2. Clients MUST ignore unrecognized extensions.
    ...
    // (Section 4.2.10). It contains the following value
    // "max_early_data_size".
    // https://tools.ietf.org/html/rfc8446#section-4.6.1
 ...
 *Finished. If the server does not send an "early_data" extension in
 *EncryptedExtensions, then the client MUST NOT send an EndOfEarlyData
 *message. This message indicates that all 0-RTT application_data messages, if
 *any, have been transmitted and that the following records are protected
 *under handshake traffic keys. Servers MUST NOT send this message, and
 *clients receiving it MUST terminate the connection with an
 *"unexpected_message" alert. This message is encrypted under keys derived
 ...
 * immediately after the ServerHello message. This is the first message that
 *is encrypted under keys derived from the server_handshake_traffic_secret.
 * The EncryptedExtensions message contains extensions that can be protected,
 *any which are not needed to establish the cryptographic context but which
 *are not associated with individual certificates. The client MUST check
 * EncryptedExtensions for the presence of any forbidden extensions and if any
 *are found MUST abort the handshake with an "illegal_parameter" alert.
 ********************************************************************************/
...
/*********************************************************************************
 * This message conveys the endpoint's certificate chain to the peer. The
 *server MUST send a Certificate message whenever the agreed-upon key exchange
 *method uses certificates for authentication (this includes all key exchange
 *methods defined in this document except PSK). The client MUST send a
 *Certificate message if and only if the server has requested client
 *authentication via a CertificateRequest message. If the server requests
 *client authentication but no suitable certificate is available, the client
 *MUST send a Certificate message containing no certificates (i.e., with the
 *"certificate_list" field having length 0). A Finished message MUST be sent
 * regardless of whether the Certificate message is empty.
 ********************************************************************************/
struct certificate_message {
    // A sequence of Certificates, each containing a single certificate and
    // set of extensions. https://tools.ietf.org/html/rfc8446#section-4.4.2
 ...
 *client, if it is appropriate for the selected cipher suite. This message, if
...
struct certificate_req_message {
    ...
    // authorization space. If the certificate_authorities list is empty then
    // the client MAY send any certificate of the appropriate
    // ClientCertificateType, unless there is some external arrangement to the
    // contrary. https://tools.ietf.org/html/rfc4346#section-7.4.4
 ...
 *a certificate from the client. This message, if sent, MUST follow
 * EncryptedExtensions.
 * https://tools.ietf.org/html/rfc8446#section-4.3.2
 ********************************************************************************/
...
/*********************************************************************************
 * The server hello done message is sent by the server to indicate the end of
 *the server hello and associated messages. After sending this message, the
 *server will wait for a client response. This message means that the server
 *is done sending messages to support the key exchange, and the client can
 *proceed with its phase of the key exchange. Upon receipt of the server hello
 *done message, the client SHOULD verify that the server provided a valid
 *certificate, if required and check that the server hello parameters are
 *acceptable. https://tools.ietf.org/html/rfc4346#section-7.4.5
 ********************************************************************************/
 ...
 *key exchange message. https://tools.ietf.org/html/rfc4346#section-7.4.8
 ********************************************************************************/
 ...
 * certificate that contained a Diffie-Hellman public key whose parameters
 *(group and generator) matched those specified by the server in its
 ...
 *algorithms, keys, and secrets. Recipients of finished messages MUST verify
 *that the contents are correct. Once a side has sent its Finished message and
 *received and validated the Finished message from its peer, it may begin to
 ...
 *information contained in the extension when selecting an OCSP responder and
 *SHOULD include request_extensions in the OCSP request. Servers return a
 *certificate response along with their certificate by sending a
 * "CertificateStatus" message immediately after the "Certificate" message
 *(and before any "ServerKeyExchange" or "CertificateRequest" messages). If a
 *server returns a "CertificateStatus" message, then the server MUST have
 *included an extension of type "status_request" with empty "extension_data"
 *in the extended server hello.
 * https://tools.ietf.org/html/rfc4366#section-3.6
 ********************************************************************************/
 ...
 *peer after it has sent a Finished message. Implementations  that receive a
 *KeyUpdate message prior to receiving a Finished message MUST terminate the
 *connection with an "unexpected_message" alert. After sending a KeyUpdate
 *message, the sender SHALL send all its traffic using the next generation of
 *keys, computed as described in Section 7.2. Upon receiving a KeyUpdate, the
 *receiver MUST update its receiving keys.
 * https://tools.ietf.org/html/rfc8446#section-4.6.3
 ********************************************************************************/
struct key_update_message {
    // Indicates whether the recipient of the KeyUpdate should respond with
    // its own KeyUpdate. If an implementation receives any other value, it
